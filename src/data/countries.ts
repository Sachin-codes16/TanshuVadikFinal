...
const getFlagUrl = (iso2: string): string => {
  const match = Object.entries(flagModules).find(([path]) => path.endsWith(`/${iso2.toUpperCase()}.svg`));
...
export const COUNTRIES: Country[] = RAW_COUNTRIES.map(([name, iso2, dialCode]) => ({
  name,
  iso2,
  dialCode,
  flagUrl: getFlagUrl(iso2),
}));
